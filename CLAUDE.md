...
| test_risk_manager.cpp | 128 | Magnitude, confidence, rate, drawdown, OMS gates, evaluate_with_reason, get_most_blocked_gate, pnl counter |
| test_trade_signal_engine.cpp | 128 | Signals, fields, backtest, cooldown, reset, stats accessors, signal quality, to_json, flush_sinks, zero-conf edge |
| test_latency_controller.cpp | 90 | Stats, percentiles, p50, pressure, backoff, reset, concurrent, get_total_latency_us |
| test_config.cpp | 68 | YAML parse, missing fields, hot-reload, risk_thresholds, pressure validation, set_token_interval_ms |
...
| test_fix_oms_adapter.cpp | 25 | FIX 4.2 session management, heartbeats, sequence recovery |
| test_oms_adapter.cpp | 24 | Mock/REST/FIX OMS adapter full coverage |
| test_prometheus_exporter.cpp | 21 | Start/stop, double-start, scrape serves metrics, no-callback |
